  ...
  return <>
    <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: scale }} transition={{ duration: 0.3, ease: [0, 0.71, 0.2, 1.01] }} className={"flex justify-center items-center h-screen"}>
      <div className={"w-96 mx-4"}>
        ...
        <input className={"input w-full mb-4"} type="password" placeholder={"Password"} />
        <input className={"input w-full mb-4"} type="password" placeholder={"Confirm your password"} />
        <div className={"flex justify-end gap-4 mb-4"}>
          <button className={"button-dark w-full"}>Register 👉</button>
        </div>
        <div className="w-full text-end">
          <button onClick={switchPage} className="underline text-blue-600">I have an account</button>
        </div>
      </div>
    </motion.div>
  </>
